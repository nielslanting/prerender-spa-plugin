  ...
  if (status !== 'success') {
    ...
    if (options.captureAfterElementExists) {
      setInterval(function () {
        var html = page.evaluate(function (elementSelector, shouldCaptureElement) {
          if (document.querySelector(elementSelector)) {
            if (shouldCaptureElement) {
              return document.querySelector(elementSelector).innerHTML;
            }
            var doctype = new window.XMLSerializer().serializeToString(document.doctype)
            var outerHTML = document.documentElement.outerHTML
            return doctype + outerHTML;
          }
        }, options.captureAfterElementExists, options.captureElement)
        if (html) returnResult(html)
      }, options.captureAfterTime || 1000)
    }
